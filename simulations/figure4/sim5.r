...
for (i in 1:100) {
  ...
  l2.edp[i] <- mean((colMeans(sim.res$pred.w.data) - truth)^2)
  l1.edp[i] <- mean(abs(colMeans(sim.res$pred.w.data) - truth))

  rm(truth)
}

save(l2.edp, l1.edp, file = "./output/thin/sim5.edp.Rdata")
